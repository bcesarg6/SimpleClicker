...
public class OvertimeButtonScript : MonoBehaviour, IPointerDownHandler, IPointerClickHandler, IPointerUpHandler {
    ...
    public void OnPointerClick(PointerEventData pointer_data) {
        if (main_script.score >= main_script.overtime_price) {
            main_script.score = main_script.score - main_script.overtime_price;
            main_script.overtime = main_script.overtime + 1;
            main_script.overtime_mod = main_script.overtime_mod + 1;
            main_script.saveData("overtime", main_script.overtime);
            main_script.saveData("overtime_mod", main_script.overtime_mod);
            main_script.attScore();
            main_script.attPrices();
            main_script.attScore();
            main_script.attOvertime();
            main_script.attShop();
        }
